...
int ocl_batcher_run(char *KERNEL_FUNCTION, int kernel_runs,
			  ...
			  uint8_t **ptr_dst)
{
  ...
  ret = clSetKernelArg(ocl_data.kernel, 2, sizeof(cl_mem), (void *)&ocl_data.src_device_clmem);
  ret = clSetKernelArg(ocl_data.kernel, 3, sizeof(cl_mem), (void *)&ocl_data.dst_device_clmem);

	size_t local_size[3] = {16, 16, 1};
	size_t global_size[3] = {imgWidth, imgHeight, 1};
	...
	{
		ret = clEnqueueNDRangeKernel(ocl_data.command_queue, ocl_data.kernel, 3, NULL, global_size, local_size, 0, NULL, &event_kernel[run]);
	}

	clWaitForEvents(1, &event_kernel[kernel_runs-1]);
	...
	cl_ulong ocl_kernel_start, ocl_kernel_end;
	clGetEventProfilingInfo(event_kernel[kernel_runs-1], CL_PROFILING_COMMAND_START, sizeof(ocl_kernel_start), &ocl_kernel_start, NULL);
	clGetEventProfilingInfo(event_kernel[kernel_runs-1], CL_PROFILING_COMMAND_END, sizeof(ocl_kernel_end), &ocl_kernel_end, NULL);
	double ocl_kernel_ns = (ocl_kernel_end-ocl_kernel_start);
	if (ret != CL_SUCCESS) {
	...
	*ptr_dst = (uint8_t*)(ocl_data.dst_host);
	...
	printf("OpenCL %s runs: %d, total time: %f ms, single kernel time: %f ms, ocl kernel time: %f ms, MPixel/s: %f\n",
			KERNEL_FUNCTION, kernel_runs, (elapsed/1000000.0), (elapsed_k/1000000.0), (ocl_kernel_ns/1000000.0), perf);
