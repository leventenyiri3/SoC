...
int32_t main(int32_t argc, char *argv[])
{
	...
	int8_t filter_laplace_int8[FILTER_H*FILTER_W];
	for (int32_t row=0; row<FILTER_H; row++) {
		for (int32_t col=0; col<FILTER_W; col++) {
			filter_laplace_int8[row*FILTER_W + col] = (int8_t)(filter_laplace_f[row*FILTER_W + col]);
		}
	}
...
#if 1
	ts_start = get_ts_ns();
	for (int32_t run=0; run<number_of_runs_scalar; run++)
	{
		fir2d_c(imgHeight,
				imgWidth,
				imgWidthF,
	...
	perf = 1000.0*double(imgHeight*imgWidth)*double(number_of_runs_scalar)/elapsed;
	printf("CPU scalar time: %f ms, MPixels/s: %f\n", (elapsed/1000000.0), perf);
#endif
	...
	printf("CPU NEON time: %f ms, MPixels/s: %f\n", (elapsed/1000000.0), perf);
#endif
	...
	perf = 1000.0*double(imgHeight*imgWidth)/elapsed;
...
#if 1
	ocl_fir2d_run("fir2d_kernel_gl", number_of_runs_kernel,
...
#if 1
	ocl_fir2d_run("fir2d_kernel_gl_16x", number_of_runs_kernel,
	              imgHeight, imgWidth, imgWidthF,
				  FILTER_W, FILTER_H,
				  filter_laplace_int8, filter_laplace_f,
				  &imgRes);
...
#if 1
	ocl_fir2d_run("fir2d_kernel_sh_uchar_int", number_of_runs_kernel,
	              imgHeight, imgWidth, imgWidthF,
				  FILTER_W, FILTER_H,
				  filter_laplace_int8, filter_laplace_f,
				  &imgRes);
...
#if 1
	ocl_fir2d_run("fir2d_kernel_sh_uchar_float", number_of_runs_kernel,
	              imgHeight, imgWidth, imgWidthF,
				  FILTER_W, FILTER_H,
				  filter_laplace_int8, filter_laplace_f,
				  &imgRes);
...
#if 1
	ocl_fir2d_run("fir2d_kernel_sh_float_float", number_of_runs_kernel,
	              imgHeight, imgWidth, imgWidthF,
				  FILTER_W, FILTER_H,
				  filter_laplace_int8, filter_laplace_f,
				  &imgRes);
...
#if 1
	ocl_fir2d_run("fir2d_kernel_sh_float_float_nbc", number_of_runs_kernel,
	              imgHeight, imgWidth, imgWidthF,
				  FILTER_W, FILTER_H,
				  filter_laplace_int8, filter_laplace_f,
				  &imgRes);
